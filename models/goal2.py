...
class BigGoalHouseModel2(nn.Module):
...
class AuxiliaryBigGoalHouseModel2(BigGoalHouseModel2):
    def __init__(self, num_inputs, num_outputs):
...
class BigGoalHouseModel4(nn.Module):
    ...
    def __init__(self, num_inputs, num_outputs):
        ...
        self.conv_merge2 = TimeDistributed(nn.Sequential(
            Flatten(),
            nn.Linear(9 ** 2 * 32, 512),
            nn.ReLU()
        ))

        self.lstm_side = TimeDistributed(nn.Sequential(
            nn.Linear(512, 512),
            nn.ReLU(),
        ))

        self.main_output_size = 1024
    ...
    def _forward_base(self, inputs, masks, states):
        ...
        lstm_features = torch.cat((lstm_features, last_reward_action,), dim = 2)
        lstm_features, states = self.rnn(lstm_features, masks, states)

        side_lstm_features = self.lstm_side(self.conv_merge2(features))
        features = torch.cat((lstm_features, side_lstm_features), 2)       
        return features, states
